...
void ncclDebugLog(ncclDebugLogLevel level, unsigned long flags, const char *filefunc, int line, const char *fmt, ...) {
  ...
  if (level == NCCL_LOG_WARN) {
    ...
    len = snprintf(buffer, sizeof(buffer), "%s:%d:%d [%d] %s:%d NCCL INFO ",hostname, pid, tid, cudaDev, filefunc, line);
    ...
    len = snprintf(buffer, sizeof(buffer), "%s:%d:%d [%d] %s:%d NCCL MOD ",hostname, pid, tid, cudaDev, filefunc, line);
  }
